...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Filtre par grosseur (corrigé)
    if (selectedGrosseur !== 'Toutes') {
      filtered = filtered.filter(s => s.grosseur === selectedGrosseur)
    }
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF9]">
      ...
      {/* BANDEAU PROMOTIONNEL 1 - OFFRE DE BIENVENUE */}
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex flex-wrap gap-2">
                {tailles.map((t) => (
                  <button 
                    ...
                    className={`px-3 py-1 rounded-full text-sm transition ${selectedTaille === t ? 'border-2 border-[#C2185B] bg-[#C2185B]/5 text-[#C2185B]' : 'border border-stone-200 hover:border-[#C2185B] hover:text-[#C2185B]'}`}
                  >
                    {t}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-2">
                {grosseurs.map((g) => (
                  <button 
                    ...
                    className={`px-3 py-1 rounded-full text-sm transition ${selectedGrosseur === g ? 'border-2 border-[#C2185B] bg-[#C2185B]/5 text-[#C2185B]' : 'border border-stone-200 hover:border-[#C2185B] hover:text-[#C2185B]'}`}
                  >
                    {g}
                  </button>
                ))}
        ...
        <div className="flex-1">
          ...
          {/* BANDEAU PROMOTIONNEL 2 - NOUVEAUTÉ */}
          {filteredServices.length > 0 && (
            <div className="mb-6 bg-gradient-to-r from-[#C2185B]/10 to-[#C2185B]/5 rounded-xl p-4 border border-[#C2185B]/20">
              <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
                ...
                <Link href="/service/3" className="px-5 py-2 bg-[#C2185B] text-white rounded-full text-sm font-semibold hover:bg-[#9b0044] transition shadow-md">
                  Découvrir →
                </Link>
              </div>
            </div>
          ...
          {filteredServices.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl shadow-sm">
              ...
              <button onClick={resetFilters} className="mt-4 text-[#C2185B] font-semibold">
                Voir toutes les coiffures
              </button>
            </div>
          ) : (
              ...
              {filteredServices.map((service) => (
                <div key={service.id} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition group">
                  <div className="relative h-80 overflow-hidden">
                    ...
                    />
                    {service.badge && (
                      <div className={`absolute top-3 right-3 px-2 py-1 rounded-full text-xs font-bold shadow-sm ${service.badge === 'Populaire' ? 'bg-white/90 backdrop-blur text-[#C2185B]' : 'bg-[#C2185B] text-white'}`}>
                        {service.badge}
                      </div>
                    )}
                    <div className="absolute bottom-3 left-3 bg-[#C2185B] text-white px-2 py-0.5 rounded-full text-xs font-bold shadow-md">
                      -10%
                    </div>
                  </div>
                  <div className="p-5">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="text-right">
                        <span className="text-[#C2185B] font-bold text-xl">{service.prix}€</span>
                    ...
                    <Link 
                      ...
                      className="w-full mt-4 py-2.5 bg-[#C2185B] text-white rounded-full block text-center font-semibold hover:bg-[#9b0044] transition shadow-md"
                    >
                      Réserver
                    </Link>
                  </div>
      ...
      <footer className="py-10 px-4 text-center bg-gradient-to-r from-[#C2185B] to-[#9b0044] text-white mt-8">
        <Link href="/" className="text-2xl font-serif italic text-white mb-4 inline-block hover:text-pink-100 transition">
          GlowBook
        </Link>
