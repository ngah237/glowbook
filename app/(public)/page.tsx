...
const couleursDisponibles = [
  ...
  { nom: 'Châtain', code: '#4B3621' },
  { nom: 'Blond miel', code: '#D4AF37' },
  { nom: 'Ombré', code: 'linear-gradient(135deg, #2C1E12, #8B4513)' },
  { nom: 'Rouge cuivré', code: '#B87333' },
...
export default function Home() {
  ...
  const [sortBy, setSortBy] = useState('popularite') // 'popularite', 'prix_asc', 'prix_desc'
  const [currentPage, setCurrentPage] = useState(1)
  const itemsPerPage = 8
  
  // Services filtrés
  const [filteredServices, setFilteredServices] = useState(servicesData)
  const [paginatedServices, setPaginatedServices] = useState(servicesData.slice(0, itemsPerPage))
  ...
  useEffect(() => {
    ...
    if (selectedCategorie !== 'Toutes') {
      filtered = filtered.filter(s => s.categorie === selectedCategorie)
    }
    
    // Filtre taille
    if (selectedTaille !== 'Toutes') {
      filtered = filtered.filter(s => s.taille === selectedTaille)
    }
    
    // Filtre prix
    filtered = filtered.filter(s => s.prix <= prixMax)
    
    // Filtre couleur (simulé - dans une vraie BDD chaque service aurait une couleur)
    if (selectedCouleur) {
    ...
    setCurrentPage(1)
  }, [searchTerm, selectedCategorie, selectedTaille, selectedGrosseur, prixMax, sortBy, selectedCouleur])

  // Pagination
  useEffect(() => {
    const start = (currentPage - 1) * itemsPerPage
    const end = start + itemsPerPage
    setPaginatedServices(filteredServices.slice(start, end))
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 flex flex-col lg:flex-row gap-8">
          ...
          <div className="sticky top-28 space-y-6">
            ...
            <div>
              <h3 className="font-semibold text-lg mb-3 flex items-center gap-2">
                <span className="material-symbols-outlined text-[#C2185B]">search</span>
                Recherche
              </h3>
              <input 
                type="text" 
                placeholder="Box braids, locks..." 
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full px-4 py-3 rounded-lg border border-stone-200 focus:ring-2 focus:ring-[#C2185B]/20 focus:border-[#C2185B] outline-none" 
              />
            </div>
            ...
            <div>
              <h3 className="font-semibold text-lg mb-3 flex items-center gap-2">
                <span className="material-symbols-outlined text-[#C2185B]">category</span>
                Catégories
              </h3>
                ...
                {categories.map((cat) => (
                  <label key={cat} className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="radio" 
                      name="categorie" 
                      checked={selectedCategorie === cat}
                      onChange={() => setSelectedCategorie(cat)}
                ...
                {tailles.map((t) => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-sm transition ${
                      selectedTaille === t 
                        ? 'border-2 border-[#C2185B] bg-[#C2185B]/5 text-[#C2185B]' 
                        : 'border border-stone-200 hover:border-[#C2185B] hover:text-[#C2185B]'
                    }`}
                  >
                    {t}
                  </button>
                ...
                {grosseurs.map((g) => (
                  <label key={g} className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="radio" 
                      name="grosseur" 
                      checked={selectedGrosseur === g}
                      onChange={() => setSelectedGrosseur(g)}
                      className="text-[#C2185B] focus:ring-[#C2185B]/20" 
                    />
            ...
            <div>
              <h3 className="font-semibold text-lg mb-3 flex items-center gap-2">
                <span className="material-symbols-outlined text-[#C2185B]">palette</span>
                Couleurs
              </h3>
              <div className="flex flex-wrap gap-2">
                {couleursDisponibles.map((couleur) => (
                  <button
                    key={couleur.nom}
                    onClick={() => setSelectedCouleur(selectedCouleur === couleur.nom ? null : couleur.nom)}
                    className={`w-8 h-8 rounded-full transition-all ${
                      selectedCouleur === couleur.nom ? 'ring-2 ring-offset-2 ring-[#C2185B] scale-110' : ''
                    }`}
                    style={{ background: couleur.code }}
                    title={couleur.nom}
                  />
                ))}
              </div>
            ...
            <div>
              <h3 className="font-semibold text-lg mb-3 flex items-center gap-2">
                <span className="material-symbols-outlined text-[#C2185B]">euro</span>
                Prix Max
              </h3>
              <input 
                type="range" 
              ...
              <div className="flex justify-between mt-2">
                ...
                <span className="text-sm font-semibold text-[#C2185B]">{prixMax}€</span>
                <span className="text-sm text-stone-500">350€</span>
              </div>
            </div>
            ...
            <button 
              ...
              className="w-full py-2 border border-[#C2185B]/30 text-[#C2185B] rounded-full text-sm font-medium hover:bg-pink-50 transition"
            >
        ...
        <div className="flex-1">
          ...
          <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
            <div>
              <span className="text-sm text-[#C2185B] uppercase font-semibold">Notre Collection</span>
              <h2 className="text-3xl font-serif font-bold">Coiffures Signature</h2>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-stone-500 text-sm">{filteredServices.length} styles</span>
              {/* 🔄 NOUVEAU : Selecteur de tri */}
              <select 
                value={sortBy} 
                onChange={(e) => setSortBy(e.target.value)}
                className="px-3 py-2 border border-stone-200 rounded-lg text-sm bg-white focus:ring-2 focus:ring-[#C2185B]/20"
              >
          ...
          {filteredServices.length === 0 ? (
            ...
            <>
              {/* Grille des services avec pagination */}
                      ...
                      {service.badge && (
                        <div className={`absolute top-3 right-3 px-2 py-1 rounded-full text-xs font-bold shadow-sm ${
                          service.badge === 'Populaire' 
                            ? 'bg-white/90 backdrop-blur text-[#C2185B]' 
              ...
              {totalPages > 1 && (
                <div className="flex justify-center gap-2 mt-8">
                  <button
                    ...
                    className="px-4 py-2 border border-stone-200 rounded-lg text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-stone-50 transition"
                  >
                    ← Précédent
                  </button>
